...
export function parseReplayEvents(events: ReplayEvent[]): ParsedReplay {
  const turnMap = new Map<
    number,
    { startTime: string; endTime?: string; nodes: AuditNode[] }
  >()
    ...
    switch (event.type) {
      ...
      case "tool_result": {
        const existing = event.tool_call_id
          ? toolMap.get(event.tool_call_id)
          : undefined
        if (existing) {
      ...
      case "error": {
        ...
        current.systemEvents.push({
          index: i,
          kind: "error",
          content: event.message ?? event.code,
        })
        break
      }
...
function buildAuditNode(
  ...
  switch (event.type) {
    ...
    case "tool_result": {
      const existing = event.tool_call_id
        ? toolStartMap.get(event.tool_call_id)
        : undefined
      if (existing) {
...
export function buildVisibleEntries(
  ...
  for (const entry of entries) {
    if (
      entry.textDeltas.length === 0 &&
      entry.reasoningDeltas.length === 0 &&
      entry.toolCalls.length === 0 &&
      entry.systemEvents.length === 0
    )
      continue

    const visibleTextDeltas = entry.textDeltas.filter(
      (d) => d.index <= currentIndex
    )
    const visibleReasoningDeltas = entry.reasoningDeltas.filter(
      (d) => d.index <= currentIndex
    )
    const visibleToolCalls = entry.toolCalls.filter(
      (tc) => tc.startIndex <= currentIndex
    )

    if (
      visibleTextDeltas.length === 0 &&
      visibleReasoningDeltas.length === 0 &&
      visibleToolCalls.length === 0 &&
      entry.systemEvents.filter((s) => s.index <= currentIndex).length === 0
    ) {
      continue
    }

    const content = visibleTextDeltas.map((d) => d.content).join("")
    const reasoningContent = visibleReasoningDeltas
      .map((d) => d.content)
      .join("")
    const lastDeltaIndex =
      visibleTextDeltas.length > 0
        ? visibleTextDeltas[visibleTextDeltas.length - 1].index
        : -1
    const isStreaming =
      lastDeltaIndex === currentIndex &&
      currentIndex <
        (entry.textDeltas[entry.textDeltas.length - 1]?.index ?? Infinity)
    ...
    const toolCalls = visibleToolCalls.map((tc) => ({
      ...
      status: (tc.resultIndex != null && tc.resultIndex <= currentIndex
        ? "completed"
        : "running") as "running" | "completed",
