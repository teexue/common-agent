...
export function AuditTreeNode({
  ...
  const visibleCount = turnNode.nodes.filter(
    (n) => nodeEventIndex(n) <= currentIndex
  ).length
  const hasActive = visibleCount > 0
  const allDone =
    turnNode.nodes.length > 0 && visibleCount === turnNode.nodes.length
    ...
    <div className="space-y-1">
      ...
      >
        ...
        <span className="flex-1 text-xs font-medium">
          {t("replay.turn", { turn: turnNode.turn })}
        </span>
  ...
  switch (kind) {
    case "compaction":
      return {
        icon: Zap,
        color: "text-warning",
        label: t("replay.compaction"),
      }
    case "sub_agent_start":
      return {
        icon: GitBranch,
        color: "text-chart-2",
        label: t("replay.subAgentStart"),
      }
    case "sub_agent_end":
      return {
        icon: GitMerge,
        color: "text-success",
        label: t("replay.subAgentEnd"),
      }
    case "error":
      return {
        icon: AlertCircle,
        color: "text-destructive",
        label: t("replay.error"),
      }
    case "done":
      return {
        icon: CheckCircle,
    ...
    case "approval_required":
      return {
        icon: Shield,
        color: "text-warning",
        label: t("replay.pendingApproval"),
      }
    default:
      return { icon: Bot, color: "text-muted-foreground", label: kind }
  }
}
