      ...
      <div className="border-t border-border px-3 py-2">
        <div className="flex flex-col gap-1.5">
          ...
          {toolCall.status === "error" && outputPreview && (
            <div>
              <span className="text-[10px] font-semibold tracking-wider text-destructive uppercase">
                {t("common.error")}
              </span>
...
function StatusIcon({
  ...
  status: string
  config: { icon: typeof Clock; color: string; bg: string; label: string }
}) {
  if (status === "running" || status === "sub_agent_running")
    return <Loader2 className={cn("h-3.5 w-3.5 animate-spin", config.color)} />
  if (status === "denied" || status === "pending_approval")
    return <config.icon className={cn("h-3.5 w-3.5", config.color)} />
          ...
          <div className="ml-auto flex shrink-0 items-center gap-1.5">
            ...
            <Badge
              ...
              className={cn(
                ...
                toolCall.status === "completed" && "bg-success/10 text-success",
                toolCall.status === "error" &&
                  "bg-destructive/10 text-destructive",
                toolCall.status === "running" && "bg-primary/10 text-primary",
