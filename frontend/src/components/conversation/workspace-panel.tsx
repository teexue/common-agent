...
export function WorkspacePanel({
  ...
  useEffect(() => {
    if (searchResults.length > 0 && matchRefs.current[currentMatch]) {
                  ...
                  {noAgent ? (
                    <>
                      ...
                      <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                        创建一个 Agent YAML 配置文件来开始使用。可以通过左侧
                        Agent 列表的「+」按钮创建，或运行{" "}
                        <code className="rounded bg-muted px-1 py-0.5 font-mono">
                          agent-server config init
                        </code>{" "}
                        进行初始化。
                      </p>
                      {onCreateAgent && (
                  ...
                  <div className="flex justify-end gap-1">
                    ...
                    <DropdownMenu>
                      ...
                      >
                        <DropdownMenuItem
                          onClick={() => {
                            const md = exportToMarkdown(messages, agentName)
                            downloadFile(
                              md,
                        ...
                        <DropdownMenuItem
                          onClick={() => {
                            const json = exportToJson(messages, agentName)
                            downloadFile(
                              json,
