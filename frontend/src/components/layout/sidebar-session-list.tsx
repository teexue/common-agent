...
function SessionListItem({
  sess, active, onResume, onDelete, onReplay,
}: {
    ...
    <div className={`group flex items-center gap-2 rounded-xl px-2.5 py-2 text-left transition-all ${active ? "bg-primary/8 text-primary" : "text-sidebar-foreground hover:bg-sidebar-accent"}`}>
      <button onClick={() => onResume?.(sess.id)} className="flex min-w-0 flex-1 items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <span className="block truncate text-xs font-medium">{sess.agent}</span>
          <span className="block truncate text-[10px] text-muted-foreground">{formatRelativeTime(sess.updated_at)}</span>
        </div>
      </button>
      <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
        {onReplay && (
          <SessionActionBtn tooltip="回放会话" onClick={(e) => { e.stopPropagation(); onReplay(sess.id) }}>
            <Play className="h-3 w-3 text-muted-foreground" />
          </SessionActionBtn>
        )}
        {onDelete && (
          <SessionActionBtn tooltip="删除会话" onClick={(e) => { e.stopPropagation(); onDelete(sess.id) }}>
...
export function SessionList({
  sessions, activeSessionId, onResumeSession, onDeleteSession, onReplaySession,
}: SessionListProps) {
  ...
  return (
    <div className="p-2.5">
      <div className="mb-2 flex items-center gap-1.5 px-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/70">
        <Clock className="h-3 w-3" /> 历史会话
      </div>
      <div className="flex flex-col gap-0.5">
        {sessions.map((sess) => (
          <SessionListItem key={sess.id} sess={sess} active={activeSessionId === sess.id} onResume={onResumeSession} onDelete={onDeleteSession} onReplay={onReplaySession} />
        ))}
      </div>
    </div>
  )
