...
import { Check, History, RotateCcw, ScrollText, Trash2, X } from "lucide-react"
...
import { RequestLogsDialog } from "@/components/audit/request-logs-dialog"
        ...
        <DialogContent className="max-w-lg rounded-2xl border-border bg-card">
          ...
          <div className="max-h-[70vh] space-y-3 overflow-y-auto pr-1 text-xs">
            ...
            {item.result && (
              <div className="space-y-1">
                <p className="text-muted-foreground">
                  {t("kanban.resultLabel")}
                </p>
                <div className="max-h-80 overflow-y-auto rounded-lg bg-muted/40 px-3 py-2 leading-relaxed text-foreground">
                  <MarkdownRenderer content={item.result} isStreaming={false} />
                </div>
              </div>
            )}
            ...
            <div className="space-y-1 rounded-xl border border-border/60 px-3 py-2">
              ...
              {item.workdir && (
                <MetaRow
                  label={t("kanban.fieldWorkdir")}
                  value={item.workdir}
                />
              )}
              {item.due_at && (
                <MetaRow
                  label={t("kanban.dueAt")}
                  value={formatTime(item.due_at)}
                />
              )}
              <MetaRow
