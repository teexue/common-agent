      ...
      <ScrollArea className="h-full">
        <div className="flex flex-col gap-4 p-4">
          <div>
            ...
            <h3 className="mt-0.5 font-mono text-sm font-semibold text-foreground">
              {toolCall.name}
            </h3>
          </div>
    ...
    return (
      <ScrollArea className="h-full">
        <div className="flex flex-col gap-4 p-4">
          <div>
            <p className="text-xs font-medium text-muted-foreground">
              {entry.role === "user" ? "用户消息" : "助手回复"}
            </p>
            <h3 className="mt-0.5 font-heading text-sm capitalize text-foreground">
              {entry.role}
            </h3>
          </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {entry.toolCalls.map((tc) => (
                  <JsonViewer key={tc.id} data={tc} title={tc.name} />
                ))}
              </div>
            </div>
          )}
